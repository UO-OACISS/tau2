        ...
        try {
            ...
            for (Enumeration e = v.elements(); e.hasMoreElements();) {
                ...
                for (int i = 0; i < files.length; i++) {
                    ...
                    // boundaries between
                    // instrumented sections to be the line "Instrumented
                    // section:"
                    // so we copy inclusive -> exclusive at this time, but we
                    // don't
                    ...
                    while (inputString != null) {
                        if (inputString.length() == 0) {
                            ...
                            if (!first && !exclusiveSet) {
                                // there was no exclusive time for this
                                // instrumented section, this means
                                // that the exclusive time = inclusive time (it
                                // did not call any
                                // instrumented functionProfiles.
                                // copy inclusive over to exclusive
                                functionProfile.setExclusive(timeMetric,
                                        functionProfile.getInclusive(timeMetric));
                                if (functionProfile.getExclusive(timeMetric) > function.getMaxExclusive(timeMetric)) {
                                    function.setMaxExclusive(timeMetric,
                                            functionProfile.getExclusive(timeMetric));
                                }
                            }
                            first = false;
                            exclusiveSet = false;
                            ...
                            // allocated
                            // correctly, just toss this value out. I don't have
                            // this metric
                            // for every measurement, so the metric indexing
                            // gets all hosed up.
                            // - kevin, Nov 15, 2004
                            // processHardwareCounter(inputString);
                    ...
                    if (!first && !exclusiveSet) {
                        ...
                        // section, this means
                        // that the exclusive time = inclusive time (it did not
                        // call any
                        // instrumented functionProfiles.
                        // copy inclusive over to exclusive
                        functionProfile.setExclusive(timeMetric,
    ...
    private void initializeThread() {
        ...
        if (header2.s1 == null)
            ...
            function = this.getTrialData().addFunction(
                    header1.s0 + ", " + header2.s0 + " lines " + header2.s1, 1);
