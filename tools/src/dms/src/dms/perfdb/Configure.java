...
public class Configure {
    ...
    public Configure(String tauroot) {
		...
		this.perfdb_home = tauroot + "/tools/src/dms";
    }
	...
	try {
		...
		// configuration file in ${PerfDB_Home}/data/perfdb.cfg
	...
	try {
	    ...
	    if (tmpString.length() > 0) { 
			tau_root = tmpString;
			perfdb_home = tau_root + "/tools/src/dms";
		}
...
*/
		...
		xml_parser = perfdb_home + "/tools/src/contrib/" + xml_parser;
	}
	catch (IOException e) {
	    // todo - get info from the exception
	    System.out.println ("I/O Error occurred.");
