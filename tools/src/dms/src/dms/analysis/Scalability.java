...
public class Scalability {
	...
	public Scalability (PerfDBSession session) {
		this.db = session.db();
	}
	...
	public ScalabilityResults trials (Vector inTrials, String measurement, String function) {
		...
		buf.append("select f.name, f.trial, t.node_count * ");
		...
		buf.append("), stddev(i.");
		buf.append(measurement);
		buf.append(") from function f inner join interval_location_profile ");
		buf.append("i on f.id = i.function inner join trial t on f.trial = ");
		buf.append("t.id where f.trial in (");
		...
		buf.append(") ");
		if (function != null)
			buf.append("and f.name like '" + function + "' ");
		buf.append("group by f.name, f.trial, threads order by ");
		buf.append("f.name, threads, f.trial");
