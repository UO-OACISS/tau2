...
public class SourceManager extends JFrame {
	...
	private boolean searchLocations(SourceRegion region, File[] list) {
		for (int j = 0; j < list.length; j++) {
			if (match(region.getFilename(), list[j].getName())) {
			    //System.out.println("found it");
				SourceViewer sourceViewer = new SourceViewer(list[j],
						region);
		...
		for (int j = 0; j < list.length; j++) {
			if (list[j].isDirectory()) {
				if (searchLocations(region, list[j].listFiles())) {
					return true;
				}
			}
		}
	...
	public void showSourceCode(SourceRegion region) {
		...
		for (int i = 0; i < listModel.getSize(); i++) {
			...
			if (searchLocations(region, children)) {
				return;
			}
		}
	...
	public SourceManager(List initialElements) {
		...
		button.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent evt) {
				...
				try {
					listModel.addElement(jFileChooser.getSelectedFile()
							.getCanonicalPath());
				} catch (Exception e) {
	...
	private void addCompItem(Component c, GridBagConstraints gbc, int x, int y,
			int w, int h) {
		gbc.gridx = x;
		gbc.gridy = y;
		gbc.gridwidth = w;
