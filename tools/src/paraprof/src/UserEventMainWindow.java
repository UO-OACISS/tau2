...
public class UserEventMainWindow extends JFrame implements ActionListener, MenuListener, Observer, ChangeListener{ 
  ...
  public UserEventMainWindow(ParaProfTrial trial, boolean debug){
      try{
	...
	try{
	    if(name){
		list = sMWData.getAllThreadData(0+valueType+order);
	    }
	    else{
		list = sMWData.getAllThreadData(2+valueType+order);
	    }
	}
	catch(Exception e){
    ...
    private Vector list = new Vector(); //list[0]:The result of a call to getSMWGeneralData in StaticMainWindowData
