...
public class CallPathTextWindow extends JFrame implements ActionListener, MenuListener, Observer {
    ...
    public CallPathTextWindow(ParaProfTrial trial, int nodeID, int contextID, int threadID,
            DataSorter dataSorter, int windowType, boolean debug) {
        try {
            this.trial = trial;
            this.nodeID = nodeID;
            ...
            if (windowType == 0)
                ...
                this.setTitle("Call Path Data " + "n,c,t, " + nodeID + "," + contextID + ","
                        + threadID + " - " + trial.getTrialIdentifier(true));
            else
            ...
            //######
            panel = new CallPathTextWindowPanel(trial, nodeID, contextID, threadID, this,
                    windowType, this.debug());
    ...
    private void sortLocalData() {
        try {
            //The name selection behaves slightly differently. Thus the check
            // for it.
            if (name) {
                if (windowType == 0)
    ...
    void closeThisWindow() {
        try {
            if (this.debug) {
                System.out.println("------------------------");
                System.out.println("A total stat window for: \"" + "n,c,t, " + nodeID + ","
                        + contextID + "," + threadID + "\" is closing");
