...
DBManagerListener {
	...
	JMenuItem applyExpressionFile = new JMenuItem("Re-Apply Expression File");
	...
	try {
		...
		if (arg.equals("Exit ParaProf!")) {
			...
			for (Iterator<Experiment> it = databaseAPI.getExperimentList().iterator(); it.hasNext();) {
			    ParaProfExperiment dbExp = new ParaProfExperiment(it.next());
			    ...
			    for (Iterator<Trial> it2 = databaseAPI.getTrialList(true).iterator(); it2.hasNext();) {
	...
	if (databaseAPI != null) {
	    ...
	    boolean found = false;
	    ListIterator<Application> l = databaseAPI.getApplicationList().listIterator();
	...
	boolean found = false;
	databaseAPI.setApplication(dbApp);
	ListIterator<Experiment> l = databaseAPI.getExperimentList().listIterator();
	while (l.hasNext()) {
    ...
    public ParaProfTrial uploadTrial(ParaProfExperiment dbExp, ParaProfTrial ppTrial) throws SQLException, DatabaseException {
	DatabaseAPI databaseAPI = this.getDatabaseAPI(null);
	if (databaseAPI == null)
	    return null;
	...
	try{
		...
		if (experiment.dBExperiment()) {
		    ...
		    experiment.setApplication((ParaProfApplication) ((DefaultMutableTreeNode) selectedNode.getParent()).getUserObject());
		    DatabaseAPI databaseAPI = this.getDatabaseAPI(experiment.getDatabase());
		    if (databaseAPI != null) {
			databaseAPI.setExperiment(experiment.getID());
			if (databaseAPI.getTrialList(false) != null) {
			    ListIterator<Trial> l = databaseAPI.getTrialList(true).listIterator();//TODO: Is xml metadata required here?
			    while (l.hasNext()) {
	...
	try {
		...
		if (selectedNode == standard) {
			...
			while (l.hasNext()) {
			    ...
			    DefaultMutableTreeNode applicationNode = new DefaultMutableTreeNode(application);
			    application.setDMTN(applicationNode);
			    getTreeModel().insertNodeInto(applicationNode, selectedNode, selectedNode.getChildCount());
			}
			databaseAPI.terminate();
		    }
	...
	if (!ppTrial.loading()) {
		...
		getTreeModel().insertNodeInto(metricNode, selectedNode, selectedNode.getChildCount());
	    }
	...
	if (obj instanceof ParaProfApplication) {
	    ...
	    TrialTableModel model = new TrialTableModel(this, ppTrial, getTreeModel());
	...
	try {
	    dataSource = UtilFncs.initializeDataSource(files, fileType, fixGprofNames,range);
	    if (dataSource == null) {
	...
	addTrial(application,experiment,files,fileType,fixGprofNames,monitorProfiles,null);
	...
	case 2:
		...
		DefaultMutableTreeNode applicationNode = new DefaultMutableTreeNode(application);
		application.setDMTN(applicationNode);
		getTreeModel().insertNodeInto(applicationNode, dbNode, dbNode.getChildCount());
		return applicationNode;
	    }
	    return null;
	...
	if (applicationNode != null) {
		...
		DefaultMutableTreeNode experimentNode = new DefaultMutableTreeNode(experiment);
		experiment.setDMTN(experimentNode);
		getTreeModel().insertNodeInto(experimentNode, applicationNode, applicationNode.getChildCount());
	...
	if (experimentNode != null) {
		...
		DefaultMutableTreeNode trialNode = new DefaultMutableTreeNode(ppTrial);
		ppTrial.setDMTN(trialNode);
		getTreeModel().insertNodeInto(trialNode, experimentNode, experimentNode.getChildCount());
		return trialNode;
	    }
	    return null;
	}
