...
TAUDB_TRIAL* taudb_find_trials(boolean full, TAUDB_TRIAL* trial) {
#ifdef TAUDB_DEBUG
  printf("Calling taudb_find_trials(%d, %p)\n", full, trial);
#endif
  PGresult *res;
  int nFields;
  int i, j;
  ...
  if (full) {
    for (i = 0 ; i < nRows ; i++) {
	  taudb_get_threads(&(trials[i]));
	  taudb_get_metrics(&(trials[i]));
	  taudb_get_timers(&(trials[i]));
	  taudb_get_counters(&(trials[i]));
	  trials[i].timer_values = taudb_get_timer_values(&(trials[i]),NULL,NULL,NULL);
	  //taudb_get_counter_values(&(trials[i]));
	}
  }
