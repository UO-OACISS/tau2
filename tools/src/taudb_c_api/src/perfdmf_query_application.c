  ...
  for (i = 0; i < PQntuples(res); i++)
  {
    ...
    for (j = 0; j < nFields; j++) {
	  if (strcmp(PQfname(res, j), "id") == 0) {
		...
		application->name = taudb_create_and_copy_string(PQgetvalue(res,i,j));

	  } else {
	    application->primary_metadata[metaIndex].name = taudb_create_and_copy_string(PQfname(res,j));
	    application->primary_metadata[metaIndex].value = taudb_create_and_copy_string(PQgetvalue(res,i,j));
		metaIndex++;
	  }
	} 
