...
int main(int argc, char *argv[]){
    ...
    printf("Initializing data...\n");
        ...
        CUT_SAFE_MALLOC( h_Kernel    = (float *)malloc(KERNEL_SIZE) );
        CUT_SAFE_MALLOC( h_Data      = (float *)malloc(DATA_SIZE)   );
        CUT_SAFE_MALLOC( h_ResultCPU = (float *)malloc(DATA_SIZE)   );
        CUT_SAFE_MALLOC( h_ResultGPU = (float *)malloc(DATA_SIZE)   );
        printf("...allocating GPU memory\n");
        CUDA_SAFE_CALL( cudaMalloc((void **)&d_Kernel, DATA_SIZE) );
        ...
        CUDA_SAFE_CALL( cudaMemset(d_Kernel, 0, DATA_SIZE) );
        CUDA_SAFE_CALL( cudaMemcpy(d_Kernel, h_Kernel, KERNEL_SIZE, cudaMemcpyHostToDevice) );
        CUDA_SAFE_CALL( cudaMemcpy(d_Data,   h_Data,     DATA_SIZE, cudaMemcpyHostToDevice) );
    ...
    CUDA_SAFE_CALL( cudaMemcpy(h_ResultGPU, d_Data, DATA_SIZE, cudaMemcpyDeviceToHost) );
