...
int main(int argc, char **argv)
{
    ...
    CUDA_SAFE_CALL( cudaMalloc((void **)&d_input,  VOTE_DATA_GROUP*warp_size * sizeof(unsigned int)) );
    CUDA_SAFE_CALL( cudaMalloc((void **)&d_result, VOTE_DATA_GROUP*warp_size * sizeof(unsigned int)) );
	genVoteTestPattern(h_input, VOTE_DATA_GROUP*warp_size);
	CUDA_SAFE_CALL( cudaMemcpy(d_input, h_input, VOTE_DATA_GROUP*warp_size * sizeof(unsigned int), cudaMemcpyHostToDevice) );
	...
	printf("\tRunning <<Vote.Any>> kernel1 ...\n");
	{
		...
		CUT_CHECK_ERROR("VoteAnyKernel() execution failed\n");
		CUDA_SAFE_CALL( cudaThreadSynchronize() );
	}
	CUDA_SAFE_CALL( cudaMemcpy(h_result, d_result, VOTE_DATA_GROUP*warp_size * sizeof(unsigned int), cudaMemcpyDeviceToHost) );
	...
	printf("\tRunning <<Vote.All>> kernel2 ...\n");
	{
		...
		CUT_CHECK_ERROR("VoteAllKernel() execution failed\n");
		CUDA_SAFE_CALL( cudaThreadSynchronize() );
	}
	CUDA_SAFE_CALL( cudaMemcpy(h_result, d_result, VOTE_DATA_GROUP*warp_size * sizeof(unsigned int), cudaMemcpyDeviceToHost) );
