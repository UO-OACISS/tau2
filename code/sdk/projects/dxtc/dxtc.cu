...
int main(int argc, char** argv) 
{
    ...
    CUDA_SAFE_CALL( cudaMalloc((void**) &d_permutations, 1024 * sizeof(uint)) );
    CUDA_SAFE_CALL( cudaMemcpy(d_permutations, permutations, 1024 * sizeof(uint), 
    ...
    // Upload image.
    CUDA_SAFE_CALL( cudaMemcpy(d_data, image, memSize, cudaMemcpyHostToDevice) );

    uint blocks = ((w + 3) / 4) * ((h + 3) / 4);
    compress<<<blocks, NUM_THREADS>>>(d_permutations, d_data, (uint2 *)d_result);

    CUT_CHECK_ERROR("compress");

    CUDA_SAFE_CALL(cudaMemcpy(result, d_result, compressedSize, cudaMemcpyDeviceToHost));
    ...
    cutFree(data);
    free(image);
    CUDA_SAFE_CALL(cudaFree(d_permutations));
