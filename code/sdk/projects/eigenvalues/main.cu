...
{   
    ...
    if( 1 == user_defined) {
        ...
        CUT_CONDITION( 0 != diag_path);
        CUT_SAFE_CALL( cutReadFilef( diag_path, &(input.a), &input_data_size));

        char* sdiag_path = cutFindFilePath( "superdiagonal.dat", exec_path);
        CUT_CONDITION( 0 != sdiag_path);
        CUT_SAFE_CALL( cutReadFilef( sdiag_path, &(input.b), &input_data_size,
                                     1));
    ...
    CUDA_SAFE_CALL( cudaMalloc( (void**) &(input.g_a), 
                                sizeof(float) * mat_size));
    CUDA_SAFE_CALL( cudaMalloc( (void**) &(input.g_b_raw),
                                sizeof(float) * mat_size));

    // copy data to device
    CUDA_SAFE_CALL( cudaMemcpy( input.g_a, input.a, sizeof(float) * mat_size,
                                cudaMemcpyHostToDevice ));
    CUDA_SAFE_CALL( cudaMemcpy( input.g_b_raw, input.b, 
                                sizeof(float) * mat_size,
                                cudaMemcpyHostToDevice ));
...
cleanupInputData( InputData& input) {
    ...
    CUDA_SAFE_CALL( cudaFree( input.g_a));
    input.g_a = NULL;
    CUDA_SAFE_CALL( cudaFree( input.g_b_raw));
    input.g_b_raw = NULL;
    input.g_b = NULL;
}
