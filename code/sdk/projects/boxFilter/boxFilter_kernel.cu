...
void initTexture(int width, int height, void *pImage)
{
    ...
    CUDA_SAFE_CALL( cudaMallocArray  ( &d_array, &channelDesc, width, height )); 
    CUDA_SAFE_CALL( cudaMemcpyToArray( d_array, 0, 0, pImage, size, cudaMemcpyHostToDevice));

    CUDA_SAFE_CALL( cudaMallocArray  ( &d_tempArray,   &channelDesc, width, height )); 
    ...
    CUDA_SAFE_CALL( cudaBindTextureToArray(tex, d_array, channelDesc) );
}

extern "C"
void freeTextures()
{
    CUDA_SAFE_CALL(cudaFreeArray(d_array));
    CUDA_SAFE_CALL(cudaFreeArray(d_tempArray));
...
{
    CUDA_SAFE_CALL( cudaBindTextureToArray(tex, d_array) );
    ...
    for(int i=0; i<iterations; i++) {
        ...
        if (iterations > 1) {
            ...
            CUDA_SAFE_CALL( cudaMemcpyToArray( d_tempArray, 0, 0, d_dest, width * height * sizeof(float), cudaMemcpyDeviceToDevice));
            CUDA_SAFE_CALL( cudaBindTextureToArray(tex, d_tempArray) );
        }
    }
}
...
void boxFilterRGBA(uint *d_src, uint *d_temp, uint *d_dest, int width, int height, 
                   int radius, int iterations, int nthreads)
{
    CUDA_SAFE_CALL( cudaBindTextureToArray(rgbaTex, d_array) );
    ...
    for(int i=0; i<iterations; i++) {
        ...
        if (iterations > 1) {
            // copy result back from global memory to array
            CUDA_SAFE_CALL( cudaMemcpyToArray( d_tempArray, 0, 0, d_dest, width * height * sizeof(float), cudaMemcpyDeviceToDevice));
            CUDA_SAFE_CALL( cudaBindTextureToArray(rgbaTex, d_tempArray) );
        }
    }
}
