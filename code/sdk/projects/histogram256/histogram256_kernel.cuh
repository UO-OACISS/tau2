...
static void initHistogram256GPU(void){
    #ifdef GMEM_ATOMICS
        CUDA_SAFE_CALL( cudaMalloc((void **)&d_Result256, HISTOGRAM_SIZE ) );
    #else
        CUDA_SAFE_CALL( cudaMalloc((void **)&d_Result256, BLOCK_N * HISTOGRAM_SIZE) );
    #endif
}

//Internal memory deallocation
static void closeHistogram256GPU(void){
    CUDA_SAFE_CALL( cudaFree(d_Result256) );
}

//histogram256 CPU front-end
static void histogram256GPU(unsigned int *h_Result, unsigned int *d_Data, int dataN){
    #ifdef GMEM_ATOMICS
        CUDA_SAFE_CALL( cudaMemset(d_Result256, 0, HISTOGRAM_SIZE) );
    ...
    CUDA_SAFE_CALL( cudaMemcpy(h_Result, d_Result256, HISTOGRAM_SIZE, cudaMemcpyDeviceToHost) );
}
