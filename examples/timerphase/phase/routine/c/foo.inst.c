...
int foo(int x)
{
	...
	TAU_PHASE_CREATE_STATIC(tautimer, "int foo(int) C [{foo.c} {13,1}-{20,1}]", " ", TAU_USER);
	TAU_PHASE_START(tautimer);

{
  ...
  { int tau_ret_val =  x;  TAU_PHASE_STOP(tautimer); return (tau_ret_val); }


}
	
	TAU_PHASE_STOP(tautimer);
...
int foo1(int x)
{
	...
	TAU_PHASE_CREATE_DYNAMIC_AUTO(tautimer, "int foo1(int) C [{foo.c} {22,1}-{29,1}]", " ", TAU_USER);
	TAU_PHASE_START(tautimer);
