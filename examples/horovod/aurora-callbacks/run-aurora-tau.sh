...
if [ -z ${TAU_ROOT+x} ]; then 
	echo "Set \$TAU_ROOT to the path to your TAU installation first."
	exit 1
fi

TAU_ARCH="${TAU_ARCH:=x86_64}"
TAU_TAGS="${TAU_TAGS:=level_zero-icpx-papi-mpi-pthread-python-pdt}"
TAU_EXEC_ARGS="${TAU_EXEC_ARGS:=-l0}"

TAU_PYTHON_PATH="${TAU_ROOT}/${TAU_ARCH}/lib/shared-${TAU_TAGS}"

if [ ! -d "${TAU_PYTHON_PATH}" ]; then
	echo "Directory ${TAU_PYTHON_PATH} does not exist."
	exit 3
fi

export PYTHONPATH="${TAU_PYTHON_PATH}:${PYTHONPATH}"
...
echo "Using TAU at ${TAU_ROOT}"
echo "Using TAU tags: ${TAU_TAGS}"
echo "Using tau_exec args: ${TAU_EXEC_ARGS}"
...
--cpu-bind ${CPU_BIND} \
tau_exec -T ${TAU_TAGS} ${TAU_EXEC_ARGS} python tensorflow2_keras_mnist_tau.py
