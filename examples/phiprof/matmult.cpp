...
#endif /* TAU_MPI */
  phiprof::start( "PHIPROF: compute interchange" );
  compute_interchange(a, b, c, NRA, NCA, NCB);
  phiprof::stop( "PHIPROF: compute interchange" );
...
int main (int argc, char *argv[])
{
  ...
  phiprof::start( "PHIPROF init program" );
#ifdef TAU_MPI
  int rc = MPI_SUCCESS;
  int rank = 0;
#if defined(PTHREADS)
...
#endif /* PTHREADS */
  phiprof::stop( "PHIPROF init program" );

/* On thread 0: */
  int i;
  for (i = 0 ; i < 1 ; i++) {
  phiprof::start( "PHIPROF do work" );
  do_work();
  phiprof::stop( "PHIPROF do work" );
  phiprof::start( "PHIPROF finalize program" );
#ifdef TAU_MPI
...
#endif /* PTHREADS */
  phiprof::stop( "PHIPROF finalize program" );
