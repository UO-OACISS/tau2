...
inline std::vector<ze_device_handle_t> GetDeviceList() {
  ...
  for (auto driver : utils1::ze::GetDriverList()) {
    for (auto device : utils1::ze::GetDeviceList(driver)) {
...
inline ze_driver_handle_t GetGpuDriver() {
  ...
  std::string value = utils1::GetEnv("PTI_DEVICE_ID");
...
inline ze_device_handle_t GetGpuDevice() {
  ...
  value = utils1::GetEnv("PTI_SUB_DEVICE_ID");
  if (value.empty()) {
    return device_list[device_id];
  }
...
inline std::string GetKernelName(
  ...
  if (demangle) {
    return utils1::Demangle(name.data());
  }
