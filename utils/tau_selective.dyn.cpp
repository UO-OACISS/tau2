...
#ifdef TAU_DYNINST
void parseDyninstInstrumentationCommand(char *command, int lineno, vector<tauInstrument *>& instrumentList);
#endif
#ifndef TAU_DYNINST
...
/* -------------------------------------------------------------------------- */
int processInstrumentationRequests(char *fname, vector<tauInstrument *>& instrumentList)
{
  ...
  /* Added by Nick T */ 
    bool dyninstLoops = false;
  if( strncmp(fname, "dyninst ", 8) == 0) 
    {
      fname = (char *)fname[9]; 
      dyninstLoops = true;
    }
  ...
  while (input.getline(line, INBUF_SIZE) || input.gcount()) {
	...
	if (strcmp(inbuf, END_INSTRUMENT_SECTION) == 0)
	  break; /* Found the end of file exclude list. */  
        if ((inbuf[0] == '#') || (inbuf[0] == '\0')) continue;
#ifdef TAU_DYNINST
	parseDyninstInstrumentationCommand(inbuf, lineno, instrumentList);    
#else /* TAU_DYNINST */
