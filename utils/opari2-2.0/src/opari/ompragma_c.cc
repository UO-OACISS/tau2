...
OMPragmaC::add_descr( int n )
{
    ...
    if ( asd )
    {
        os << " POMP2_DLIST_" << std::setw( 5 ) << std::setfill( '0' ) << n;
    }
    else
    {
...
/** @brief Split combined parallel and worksharing constructs in two seperate pragmas to allow the
 *         insertion of POMP function calles between the parallel and the worksharing construct.
