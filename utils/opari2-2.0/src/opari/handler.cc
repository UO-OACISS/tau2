...
/** @brief Instrument an OpenMP Fork. The pomp_num_threads variable is used to pass the number of
 *         requested threads for the parallel region to the POMP library. It is either the result of
 *         omp_get_max_threads() or of the num_threads() clause, if present. In Fortran a wrapper function
 *         pomp_get_max_threads() is used, since it is not possible to ensure, that omp_get_max_threads is
 *         not used in the user program. We would need to parse much more of the Fortran Syntax to detect these cases.
 *         The Wrapper function avoids double definition of this function and avoids errors.*/
void
		      ...
		      OMPRegion*  r)
{
	...
	if (p->num_threads.empty()){
	  os << "int pomp_num_threads = omp_get_max_threads();\n";
		...
		os << "int pomp_num_threads = "<< p->num_threads << ";\n";
	}
       	os << " POMP2_" << c1 << ( type + 1 ) << "_" << event 
	   << "(&" << region_id_prefix << id << ", pomp_num_threads";
	if(r != 0){
	  os << ", " << r->generate_ctc_string( lang ) << " ";
	}
	os << ");\n";
...
generate_barrier( int      n,
                  ostream& os )
{
  generate_call( "enter", "implicit_barrier", n, os, NULL );
  generate_pragma( "barrier", os );
  generate_call( "exit", "implicit_barrier", n, os, NULL );
}

/**@brief Print the OpenMP Pragma. Insert num_threads clause, if needed to pass pomp_num_therads to
 * the OpenMP runtime. The num_threads clause, if it was presend, may not appear here again, since it
 * would be evaluated twice and it may have side effects.*/
void
              ...
              ostream&  os )
{
	...
	if (lang & L_F77){
		//insert the num_threads clause in a new line, since fortran lines are limited in size
		if (copytpd){
			os << p->lines.back() << "\n!$omp& num_threads(pomp_num_threads) copyin(" << pomp_tpd << ")\n";
		}else{
		...
		if (copytpd){
			os << p->lines.back() << " num_threads(pomp_num_threads) copyin(" << pomp_tpd << ")\n";
		}else{
			os << p->lines.back() << " num_threads(pomp_num_threads) \n";
		}
	}
    }else{
        // print pragma text
               ...
               ostream&  os )
{
    ...
    print_pragma( p, os );
    generate_call( "join", "parallel", n, os, NULL);
    if ( keepSrcInfo )
    {
        reset_src_info( p, os );
    }
...
{
    OMPRegion* r = REnter( p );
    cerr << "WARNING: OpenMP tasking is not supported by this version of opari2, using tasks may cause problems!\n";
    print_pragma( p, os );
    if ( keepSrcInfo )
    {
        reset_src_info( p, os );
           ...
           ostream&  os )
{
    ...
    int        n = r->id;
    regions.push_back( r );
    generate_call( "enter", "barrier", n, os, r );
    print_pragma( p, os );
    generate_call( "exit", "barrier", n, os, NULL);
    if ( keepSrcInfo )
    {
        reset_src_info( p, os );
    }
