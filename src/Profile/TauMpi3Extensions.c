...
void* recvbuf, int recvcount, MPI_Datatype recvtype, int root, MPI_Comm comm, MPI_Request* request)
{
  ...
  retvalue = PMPI_Iscatterv(sendbuf, sendcounts, displs, sendtype, recvbuf, recvcount, recvtype, root, comm,
    request);
  track_vector(TAU_ISCATTERV_DATA, sendcounts, typesize);
  TAU_PROFILE_STOP(t);
  return retvalue;
}
...
void* recvbuf, TAU_MPICH3_CONST int * recvcounts, TAU_MPICH3_CONST int * rdispls, MPI_Datatype recvtype, MPI_Comm comm, MPI_Request* request)
{
  ...
  tracksize = sum_array(sendcounts, sendtype, comm);
  tracksize += sum_array(recvcounts, recvtype, comm);
  TAU_IALLTOALLV_DATA(tracksize);
  TAU_PROFILE_STOP(t);
  return retvalue;
}
...
MPI_Comm comm, MPI_Request* request)
{
  ...
  retvalue = PMPI_Ialltoallw(sendbuf, sendcounts, sdispls, sendtypes, recvbuf, recvcounts, rdispls, recvtypes,
    comm, request);
  tracksize =  sum_array_w(sendcounts, sendtypes, comm);
  tracksize += sum_array_w(recvcounts, recvtypes, comm);
  TAU_IALLTOALLW_DATA(tracksize);
  TAU_PROFILE_STOP(t);
  return retvalue;
}
...
MPI_Comm comm, MPI_Request* request)
{
  ...
  int commRank;
  PMPI_Comm_rank(comm, &commRank);
  TAU_IREDUCESCATTER_DATA(typesize*recvcounts[commRank]);
  TAU_PROFILE_STOP(t);
  return retvalue;
}
...
MPI_Request* request)
{
  ...
  TAU_IEXSCAN_DATA(typesize*count);
  TAU_PROFILE_STOP(t);
  return retvalue;
}
...
MPI_Datatype recvtype, MPI_Comm comm)
{
  ...
  TAU_NALLGATHER_DATA(typesize*recvcount);
  TAU_PROFILE_STOP(t);
  return retvalue;
}
...
MPI_Datatype recvtype, MPI_Comm comm)
{
  ...
  TAU_NALLTOALL_DATA((typesize*recvcount)+(typesize2*sendcount));
  TAU_PROFILE_STOP(t);
  return retvalue;
}
...
MPI_Datatype recvtype, MPI_Comm comm, MPI_Request* request)
{
  ...
  TAU_INALLGATHER_DATA(typesize*recvcount);
  TAU_PROFILE_STOP(t);
  return retvalue;
}
...
MPI_Datatype recvtype, MPI_Comm comm, MPI_Request* request)
{
  ...
  TAU_INALLTOALL_DATA((typesize*recvcount)+(typesize2*sendcount));
  TAU_PROFILE_STOP(t);
  return retvalue;
}
...
void* recvbuf, TAU_MPICH3_CONST int * recvcounts, TAU_MPICH3_CONST int * rdispls, MPI_Datatype recvtype, MPI_Comm comm, MPI_Request* request)
{
  ...
  tracksize = sum_array(sendcounts, sendtype, comm);
  tracksize += sum_array(recvcounts, recvtype, comm);
  TAU_INALLTOALLV_DATA(tracksize);
  TAU_PROFILE_STOP(t);
  return retvalue;
}
...
TAU_MPICH3_CONST MPI_Datatype * recvtypes, MPI_Comm comm, MPI_Request* request)
{
  ...
  tracksize = sum_array_w(sendcounts, sendtypes, comm);
  tracksize += sum_array_w(recvcounts, recvtypes, comm);
  TAU_INALLTOALLW_DATA(tracksize);
  TAU_PROFILE_STOP(t);
  return retvalue;
}
...
TAU_MPICH3_CONST int * destinations, TAU_MPICH3_CONST int * destweights, MPI_Info info, int reorder, MPI_Comm * comm_dist_graph)
{
  int retvalue;
...
MPI_Fint * reorder, MPI_Fint * comm_dist_graph, MPI_Fint * ierr)
{
  ...
  *ierr = MPI_Dist_graph_create_adjacent(MPI_Comm_f2c(*comm_old),
    *indegree, sources, sourceweights, *outdegree,
...
******************************************************/
extern void MPI_RPUT(MPI_Aint * origin_addr, MPI_Fint * origin_count, MPI_Fint * origin_datatype, MPI_Fint * target_rank,
...
{
  MPI_Request local_request;
  *ierr = MPI_Rget_accumulate(origin_addr, *origin_count, MPI_Type_f2c(*origin_datatype), result_addr, 
...
{
  MPI_Comm local_comm_dist_graph;
  *ierr = MPI_Dist_graph_create( MPI_Comm_f2c(*comm_old), *n, sources,
