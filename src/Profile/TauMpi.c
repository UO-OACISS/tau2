...
#define track_vector( call, counts, typesize ) { \
    ...
    PMPI_Comm_rank(TAU_SOS_MAP_COMMUNICATOR(comm), &commRank); \
    PMPI_Comm_size(TAU_SOS_MAP_COMMUNICATOR(comm), &commSize); \
    if ( commRank == root ) { \
...
/* This function translates a given rank in a given communicator to the proper
   rank in TAU_SOS_MAP_COMMUNICATOR(MPI_COMM_WORLD) */
static int TauTranslateRankToWorld(MPI_Comm comm, int rank) {
  ...
  if (TAU_SOS_MAP_COMMUNICATOR(comm) != TAU_SOS_MAP_COMMUNICATOR(MPI_COMM_WORLD)) {

    int result;
    PMPI_Comm_compare(TAU_SOS_MAP_COMMUNICATOR(comm), TAU_SOS_MAP_COMMUNICATOR(MPI_COMM_WORLD), &result);
    if (result == MPI_IDENT || result == MPI_CONGRUENT) {
      ...
      PMPI_Comm_group(TAU_SOS_MAP_COMMUNICATOR(comm), &commGroup);
...
char *note;
{
...
#ifdef DEBUG
  int myrank;
  PMPI_Comm_rank(TAU_SOS_MAP_COMMUNICATOR(MPI_COMM_WORLD), &myrank);
#endif /* DEBUG */
  ...
  { /* See if we need to see the status to get values of tag & id */
    ...
    TAU_TRACE_RECVMSG(othertag, TauTranslateRankToWorld(TAU_SOS_MAP_COMMUNICATOR(rq->comm), otherid), rq->size);
...
char *note;
{
  ...
  { 
    otherid = TauTranslateRankToWorld(TAU_SOS_MAP_COMMUNICATOR(rq->comm), rq->otherParty);
    othertag = rq->tag;
...
MPI_Comm comm;
{
  ...
  TAU_TRACK_COMM(TAU_SOS_MAP_COMMUNICATOR(comm));
  returnVal = PMPI_Allgather( sendbuf, sendcount, sendtype, recvbuf, recvcount, recvtype, TAU_SOS_MAP_COMMUNICATOR(comm) );
...
MPI_Comm comm;
{
  ...
  TAU_TRACK_COMM(TAU_SOS_MAP_COMMUNICATOR(comm));
  returnVal = PMPI_Allgatherv( sendbuf, sendcount, sendtype, recvbuf, recvcounts, displs, recvtype, TAU_SOS_MAP_COMMUNICATOR(comm) );
  PMPI_Type_size( recvtype, &typesize );
...
MPI_Comm comm;
{
  ...
  TAU_TRACK_COMM(TAU_SOS_MAP_COMMUNICATOR(comm));
  returnVal = PMPI_Allreduce( sendbuf, recvbuf, count, datatype, op, TAU_SOS_MAP_COMMUNICATOR(comm) );
...
MPI_Comm comm;
{
  ...
  returnVal = PMPI_Alltoall( sendbuf, sendcount, sendtype, recvbuf, recvcnt, recvtype, TAU_SOS_MAP_COMMUNICATOR(comm) );
  PMPI_Type_size( sendtype, &typesize );
...
MPI_Comm comm;
{
  ...
  TAU_TRACK_COMM(TAU_SOS_MAP_COMMUNICATOR(comm));
  returnVal = PMPI_Alltoallv( sendbuf, sendcnts, sdispls, sendtype, recvbuf, recvcnts, rdispls, recvtype, TAU_SOS_MAP_COMMUNICATOR(comm) );

  tracksize = sum_array(sendcnts, sendtype, TAU_SOS_MAP_COMMUNICATOR(comm));
  tracksize += sum_array(recvcnts, recvtype, TAU_SOS_MAP_COMMUNICATOR(comm));
...
MPI_Comm comm;
{
  ...
  TAU_TRACK_COMM(TAU_SOS_MAP_COMMUNICATOR(comm));
  returnVal = PMPI_Barrier( TAU_SOS_MAP_COMMUNICATOR(comm) );
...
MPI_Comm comm;
{
  ...
  returnVal = PMPI_Bcast( buffer, count, datatype, root, TAU_SOS_MAP_COMMUNICATOR(comm) );
  PMPI_Type_size( datatype, &typesize );
...
MPI_Comm comm;
{
  ...
  TAU_TRACK_COMM(TAU_SOS_MAP_COMMUNICATOR(comm));
  returnVal = PMPI_Gather( sendbuf, sendcnt, sendtype, recvbuf, recvcount, recvtype, root, TAU_SOS_MAP_COMMUNICATOR(comm) );

  PMPI_Comm_rank ( TAU_SOS_MAP_COMMUNICATOR(comm), &rank );
  if (rank == root) {
    PMPI_Type_size( recvtype, &typesize );
...
MPI_Comm comm;
{
  ...
  TAU_TRACK_COMM(TAU_SOS_MAP_COMMUNICATOR(comm));
  returnVal = PMPI_Gatherv( sendbuf, sendcnt, sendtype, recvbuf, recvcnts, displs, recvtype, root, TAU_SOS_MAP_COMMUNICATOR(comm) );
...
MPI_Comm comm;
{
  ...
  TAU_TRACK_COMM(TAU_SOS_MAP_COMMUNICATOR(comm));
  returnVal = PMPI_Reduce_scatter( sendbuf, recvbuf, recvcnts, datatype, op, TAU_SOS_MAP_COMMUNICATOR(comm) );
  PMPI_Type_size( datatype, &typesize );
...
MPI_Comm comm;
{
  ...
  TAU_TRACK_COMM(TAU_SOS_MAP_COMMUNICATOR(comm));
  returnVal = PMPI_Reduce( sendbuf, recvbuf, count, datatype, op, root, TAU_SOS_MAP_COMMUNICATOR(comm) );
  PMPI_Type_size( datatype, &typesize );
...
MPI_Comm comm;
{
  ...
  TAU_TRACK_COMM(TAU_SOS_MAP_COMMUNICATOR(comm));
  returnVal = PMPI_Scan( sendbuf, recvbuf, count, datatype, op, TAU_SOS_MAP_COMMUNICATOR(comm) );
  PMPI_Type_size( datatype, &typesize );
...
MPI_Comm comm;
{
  ...
  TAU_TRACK_COMM(TAU_SOS_MAP_COMMUNICATOR(comm));
  returnVal = PMPI_Scatter( sendbuf, sendcnt, sendtype, recvbuf, recvcnt, recvtype, root, TAU_SOS_MAP_COMMUNICATOR(comm) );
  PMPI_Type_size( sendtype, &typesize );
...
MPI_Comm comm;
{
  ...
  TAU_TRACK_COMM(TAU_SOS_MAP_COMMUNICATOR(comm));
  returnVal = PMPI_Scatterv( sendbuf, sendcnts, displs, sendtype, recvbuf, recvcnt, recvtype, root, TAU_SOS_MAP_COMMUNICATOR(comm) );
...
int keyval;
{
  ...
  returnVal = PMPI_Attr_delete( TAU_SOS_MAP_COMMUNICATOR(comm), keyval );
...
int * flag;
{
  ...
  TAU_TRACK_COMM(TAU_SOS_MAP_COMMUNICATOR(comm));
  returnVal = PMPI_Attr_get( TAU_SOS_MAP_COMMUNICATOR(comm), keyval, attr_value, flag );
...
void * attr_value;
{
  ...
  TAU_TRACK_COMM(TAU_SOS_MAP_COMMUNICATOR(comm));
  returnVal = PMPI_Attr_put( TAU_SOS_MAP_COMMUNICATOR(comm), keyval, attr_value );
...
int * result;
{
  ...
  returnVal = PMPI_Comm_compare( TAU_SOS_MAP_COMMUNICATOR(comm1), TAU_SOS_MAP_COMMUNICATOR(comm2), result );
...
MPI_Comm * comm_out;
{
  ...
  returnVal = PMPI_Comm_create( TAU_SOS_MAP_COMMUNICATOR(comm), group, comm_out );
...
MPI_Comm * comm_out;
{
  ...
  TAU_TRACK_COMM(TAU_SOS_MAP_COMMUNICATOR(comm));
  returnVal = PMPI_Comm_dup( TAU_SOS_MAP_COMMUNICATOR(comm), comm_out );
...
MPI_Comm * comm;
{
  ...
  MPI_Comm silly =  TAU_SOS_MAP_COMMUNICATOR(*comm);
...
MPI_Group * group;
{
  ...
  TAU_TRACK_COMM(TAU_SOS_MAP_COMMUNICATOR(comm));
  returnVal = PMPI_Comm_group( TAU_SOS_MAP_COMMUNICATOR(comm), group );
...
int * rank;
{
  ...
  TAU_TRACK_COMM(TAU_SOS_MAP_COMMUNICATOR(comm));
  returnVal = PMPI_Comm_rank( TAU_SOS_MAP_COMMUNICATOR(comm), rank );
...
MPI_Group * group;
{
  ...
  TAU_TRACK_COMM(TAU_SOS_MAP_COMMUNICATOR(comm));
  returnVal = PMPI_Comm_remote_group( TAU_SOS_MAP_COMMUNICATOR(comm), group );
...
int * size;
{
  ...
  TAU_TRACK_COMM(TAU_SOS_MAP_COMMUNICATOR(comm));
  returnVal = PMPI_Comm_remote_size( TAU_SOS_MAP_COMMUNICATOR(comm), size );
...
int * size;
{
  ...
  returnVal = PMPI_Comm_size( TAU_SOS_MAP_COMMUNICATOR(comm), size );
...
/*   printf ("comm %p split into %p for %d\n", oldcommhandle, newcommhandle, procid_0); */
  MPI_Comm_size(TAU_SOS_MAP_COMMUNICATOR(newcomm), &newCommSize);
/*   printf ("comm %p split into %p for %d, new size = %d\n", oldcommhandle, newcommhandle, procid_0, newCommSize); */
  ...
  for (i=0; i<limit; i++) {
    worldrank = TauTranslateRankToWorld(TAU_SOS_MAP_COMMUNICATOR(newcomm), i);
...
MPI_Comm * comm_out;
{
  ...
  MPI_Comm newcomm = TAU_SOS_MAP_COMMUNICATOR(comm);
  returnVal = PMPI_Comm_split( newcomm, color, key, comm_out );

#ifdef TAU_EXP_TRACK_COMM
  tau_exp_track_comm_split(newcomm, TAU_SOS_MAP_COMMUNICATOR(*comm_out));
#endif /* TAU_EXP_TRACK_COMM */
...
int * flag;
{
  ...
  TAU_TRACK_COMM(TAU_SOS_MAP_COMMUNICATOR(comm));
  returnVal = PMPI_Comm_test_inter( TAU_SOS_MAP_COMMUNICATOR(comm), flag );
...
MPI_Comm * comm_out;
{
  ...
  returnVal = PMPI_Intercomm_create( TAU_SOS_MAP_COMMUNICATOR(local_comm), local_leader, TAU_SOS_MAP_COMMUNICATOR(peer_comm), remote_leader, tag, comm_out );
...
MPI_Comm * comm_out;
{
  ...
  TAU_TRACK_COMM(TAU_SOS_MAP_COMMUNICATOR(comm));
  returnVal = PMPI_Intercomm_merge( TAU_SOS_MAP_COMMUNICATOR(comm), high, comm_out );
...
#endif /* MPI_Abort & LAM MPI [LAM MPI] */
{
  ...
  returnVal = PMPI_Abort( TAU_SOS_MAP_COMMUNICATOR(comm), errorcode );
...
MPI_Errhandler * errhandler;
{
  ...
  TAU_TRACK_COMM(TAU_SOS_MAP_COMMUNICATOR(comm));
  returnVal = PMPI_Errhandler_get( TAU_SOS_MAP_COMMUNICATOR(comm), errhandler );
...
MPI_Errhandler errhandler;
{
  ...
  TAU_TRACK_COMM(TAU_SOS_MAP_COMMUNICATOR(comm));
  returnVal = PMPI_Errhandler_set( TAU_SOS_MAP_COMMUNICATOR(comm), errhandler );
...
int  MPI_Finalize(  )
{
  ...
  if (Tau_get_node() < 0) {
    ...
    PMPI_Comm_rank( TAU_SOS_MAP_COMMUNICATOR(MPI_COMM_WORLD), &procid_0 );
...
MPI_Comm comm;
{
  ...
  if (TauEnv_get_track_message()) {
    if (dest != MPI_PROC_NULL) {
      PMPI_Type_size( datatype, &typesize );
      TAU_TRACE_SENDMSG(tag, TauTranslateRankToWorld(TAU_SOS_MAP_COMMUNICATOR(comm), dest), typesize*count);
    }
  }
  TAU_TRACK_COMM(TAU_SOS_MAP_COMMUNICATOR(comm));
  
  returnVal = PMPI_Bsend( buf, count, datatype, dest, tag, TAU_SOS_MAP_COMMUNICATOR(comm) );
...
/* fprintf( stderr, "MPI_Bsend_init call on %d\n", procid_0 ); */
  ...
  TAU_TRACK_COMM(TAU_SOS_MAP_COMMUNICATOR(comm));
  returnVal = PMPI_Bsend_init( buf, count, datatype, dest, tag, TAU_SOS_MAP_COMMUNICATOR(comm), request );

  if (TauEnv_get_track_message()) {
    TauAddRequestData(RQ_SEND, count, datatype, dest, tag, TAU_SOS_MAP_COMMUNICATOR(comm), request, returnVal, 1);
  }
...
MPI_Request * request;
{
  ...
  TAU_TRACK_COMM(TAU_SOS_MAP_COMMUNICATOR(comm));
  returnVal = PMPI_Recv_init( buf, count, datatype, source, tag, TAU_SOS_MAP_COMMUNICATOR(comm), request );

  TAU_PROFILE_STOP(tautimer);

  if (TauEnv_get_track_message()) {
    TauAddRequestData(RQ_RECV, count, datatype, source, tag, TAU_SOS_MAP_COMMUNICATOR(comm), request, returnVal, 1);
  }
  return returnVal;
}
...
MPI_Request * request;
{
  ...
  TAU_TRACK_COMM(TAU_SOS_MAP_COMMUNICATOR(comm));
  returnVal = PMPI_Send_init( buf, count, datatype, dest, tag, TAU_SOS_MAP_COMMUNICATOR(comm), request );
...
MPI_Request * request;
{
  ...
  if (TauEnv_get_track_message()) {
    if (dest != MPI_PROC_NULL) {
      PMPI_Type_size( datatype, &typesize );
      TAU_TRACE_SENDMSG(tag, TauTranslateRankToWorld(TAU_SOS_MAP_COMMUNICATOR(comm), dest), count * typesize);
    }
  }
  
  TAU_TRACK_COMM(TAU_SOS_MAP_COMMUNICATOR(comm));
  returnVal = PMPI_Ibsend( buf, count, datatype, dest, tag, TAU_SOS_MAP_COMMUNICATOR(comm), request );
  TAU_PROFILE_STOP(tautimer);
  return returnVal;
}
...
MPI_Status * status;
{
  ...
  TAU_TRACK_COMM(TAU_SOS_MAP_COMMUNICATOR(comm));
  returnVal = PMPI_Iprobe( source, tag, TAU_SOS_MAP_COMMUNICATOR(comm), flag, status );
...
MPI_Request * request;
{
  ...
  TAU_TRACK_COMM(TAU_SOS_MAP_COMMUNICATOR(comm));
  returnVal = PMPI_Irecv( buf, count, datatype, source, tag, TAU_SOS_MAP_COMMUNICATOR(comm), request );
  ...
  if (TauEnv_get_track_message()) {
    TauAddRequestData(RQ_RECV, count, datatype, source, tag, TAU_SOS_MAP_COMMUNICATOR(comm), request, returnVal, 0);
  }
...
if (TauEnv_get_track_message()) {
  if (dest != MPI_PROC_NULL) {
    ...
    TAU_TRACE_SENDMSG(tag, TauTranslateRankToWorld(TAU_SOS_MAP_COMMUNICATOR(comm), dest), count * typesize3);
  }
}

  TAU_TRACK_COMM(TAU_SOS_MAP_COMMUNICATOR(comm));
  returnVal = PMPI_Irsend( buf, count, datatype, dest, tag, TAU_SOS_MAP_COMMUNICATOR(comm), request );
...
MPI_Request * request;
{
  ...
  TAU_TRACK_COMM(TAU_SOS_MAP_COMMUNICATOR(comm));
  returnVal = PMPI_Isend( buf, count, datatype, dest, tag, TAU_SOS_MAP_COMMUNICATOR(comm), request );
  TAU_PROFILE_STOP(tautimer);
  return returnVal;
}
...
MPI_Request * request;
{
  ...
  TAU_TRACK_COMM(TAU_SOS_MAP_COMMUNICATOR(comm));
  returnVal = PMPI_Issend( buf, count, datatype, dest, tag, TAU_SOS_MAP_COMMUNICATOR(comm), request );
  TAU_PROFILE_STOP(tautimer);
  return returnVal;
}
...
MPI_Comm comm;
{
  ...
  TAU_TRACK_COMM(TAU_SOS_MAP_COMMUNICATOR(comm));
  returnVal = PMPI_Pack( inbuf, incount, type, outbuf, outcount, position, TAU_SOS_MAP_COMMUNICATOR(comm) );
...
int * size;
{
  ...
  TAU_TRACK_COMM(TAU_SOS_MAP_COMMUNICATOR(comm));
  returnVal = PMPI_Pack_size( incount, datatype, TAU_SOS_MAP_COMMUNICATOR(comm), size );
...
MPI_Status * status;
{
  ...
  TAU_TRACK_COMM(TAU_SOS_MAP_COMMUNICATOR(comm));
  returnVal = PMPI_Probe( source, tag, TAU_SOS_MAP_COMMUNICATOR(comm), status );
...
MPI_Status * status;
{
  ...
  returnVal = PMPI_Recv( buf, count, datatype, source, tag, TAU_SOS_MAP_COMMUNICATOR(comm), status );

  if (TauEnv_get_track_message()) {
    if (source != MPI_PROC_NULL && returnVal == MPI_SUCCESS) {
      PMPI_Get_count( status, MPI_BYTE, &size );
      /* note that status->MPI_COMM must == comm */
      TAU_TRACE_RECVMSG(status->MPI_TAG, TauTranslateRankToWorld(TAU_SOS_MAP_COMMUNICATOR(comm), status->MPI_SOURCE), size);
    }
  }
  TAU_PROFILE_STOP(tautimer);
...
MPI_Comm comm;
{
  ...
  TAU_TRACK_COMM(TAU_SOS_MAP_COMMUNICATOR(comm));
  returnVal = PMPI_Rsend( buf, count, datatype, dest, tag, TAU_SOS_MAP_COMMUNICATOR(comm) );
  TAU_PROFILE_STOP(tautimer);
  return returnVal;
}
...
/* fprintf( stderr, "MPI_Rsend_init call on %d\n", procid_0 ); */
  ...
  TAU_TRACK_COMM(TAU_SOS_MAP_COMMUNICATOR(comm));
  returnVal = PMPI_Rsend_init( buf, count, datatype, dest, tag, TAU_SOS_MAP_COMMUNICATOR(comm), request );
...
MPI_Comm comm;
{
  ...
  TAU_TRACK_COMM(TAU_SOS_MAP_COMMUNICATOR(comm));
  returnVal = PMPI_Send( buf, count, datatype, dest, tag, TAU_SOS_MAP_COMMUNICATOR(comm) );
  TAU_PROFILE_STOP(tautimer);
  return returnVal;
}
...
MPI_Status * status;
{
  ...
  if (TauEnv_get_track_message()) {
    if (dest != MPI_PROC_NULL) {
      ...
      TAU_TRACE_SENDMSG(sendtag, TauTranslateRankToWorld(TAU_SOS_MAP_COMMUNICATOR(comm), dest), typesize1*sendcount);
    }
  ...
  TAU_TRACK_COMM(TAU_SOS_MAP_COMMUNICATOR(comm));
  returnVal = PMPI_Sendrecv( sendbuf, sendcount, sendtype, dest, sendtag, recvbuf, recvcount, recvtype, source, recvtag, TAU_SOS_MAP_COMMUNICATOR(comm), status );

  if (TauEnv_get_track_message()) {
    if (source != MPI_PROC_NULL && returnVal == MPI_SUCCESS) {
      PMPI_Get_count( status, MPI_BYTE, &count );
      TAU_TRACE_RECVMSG(status->MPI_TAG, TauTranslateRankToWorld(TAU_SOS_MAP_COMMUNICATOR(comm), status->MPI_SOURCE), count);
    }
  }
  TAU_PROFILE_STOP(tautimer);
...
MPI_Status * status;
{
  ...
  if (TauEnv_get_track_message()) {
    if (dest != MPI_PROC_NULL) {
      ...
      TAU_TRACE_SENDMSG(sendtag, TauTranslateRankToWorld(TAU_SOS_MAP_COMMUNICATOR(comm), dest), typesize2*count);
    }
  ...
  TAU_TRACK_COMM(TAU_SOS_MAP_COMMUNICATOR(comm));
  returnVal = PMPI_Sendrecv_replace( buf, count, datatype, dest, sendtag, source, recvtag, TAU_SOS_MAP_COMMUNICATOR(comm), status );
  
  if (TauEnv_get_track_message()) {
    if (dest != MPI_PROC_NULL && returnVal == MPI_SUCCESS) {
      PMPI_Get_count( status, MPI_BYTE, &size1 );
      TAU_TRACE_RECVMSG(status->MPI_TAG, TauTranslateRankToWorld(TAU_SOS_MAP_COMMUNICATOR(comm), status->MPI_SOURCE), size1);
    }
  }
  TAU_PROFILE_STOP(tautimer);
...
MPI_Comm comm;
{
  ...
  TAU_TRACK_COMM(TAU_SOS_MAP_COMMUNICATOR(comm));
  returnVal = PMPI_Ssend( buf, count, datatype, dest, tag, TAU_SOS_MAP_COMMUNICATOR(comm) );
...
/* fprintf( stderr, "MPI_Ssend_init call on %d\n", procid_0 ); */
  ...
  TAU_TRACK_COMM(TAU_SOS_MAP_COMMUNICATOR(comm));
  returnVal = PMPI_Ssend_init( buf, count, datatype, dest, tag, TAU_SOS_MAP_COMMUNICATOR(comm), request );
...
MPI_Comm comm;
{
  ...
  TAU_TRACK_COMM(TAU_SOS_MAP_COMMUNICATOR(comm));
  returnVal = PMPI_Unpack( inbuf, insize, position, outbuf, outcount, type, TAU_SOS_MAP_COMMUNICATOR(comm) );
...
int * coords;
{
  ...
  TAU_TRACK_COMM(TAU_SOS_MAP_COMMUNICATOR(comm));
  returnVal = PMPI_Cart_coords( TAU_SOS_MAP_COMMUNICATOR(comm), rank, maxdims, coords );
...
MPI_Comm * comm_cart;
{
  ...
  returnVal = PMPI_Cart_create( TAU_SOS_MAP_COMMUNICATOR(comm_old), ndims, dims, periods, reorder, comm_cart );
...
int * coords;
{
  ...
  returnVal = PMPI_Cart_get( TAU_SOS_MAP_COMMUNICATOR(comm), maxdims, dims, periods, coords );
...
int * newrank;
{
  ...
  TAU_TRACK_COMM(TAU_SOS_MAP_COMMUNICATOR(comm_old));
  returnVal = PMPI_Cart_map( TAU_SOS_MAP_COMMUNICATOR(comm_old), ndims, dims, periods, newrank );
...
int * rank;
{
  ...
  TAU_TRACK_COMM(TAU_SOS_MAP_COMMUNICATOR(comm));
  returnVal = PMPI_Cart_rank( TAU_SOS_MAP_COMMUNICATOR(comm), coords, rank );
...
int * dest;
{
  ...
  returnVal = PMPI_Cart_shift( TAU_SOS_MAP_COMMUNICATOR(comm), direction, displ, source, dest );
...
MPI_Comm * comm_new;
{
  ...
  TAU_TRACK_COMM(TAU_SOS_MAP_COMMUNICATOR(comm));
  returnVal = PMPI_Cart_sub( TAU_SOS_MAP_COMMUNICATOR(comm), remain_dims, comm_new );
...
int * ndims;
{
  ...
  TAU_TRACK_COMM(TAU_SOS_MAP_COMMUNICATOR(comm));
  returnVal = PMPI_Cartdim_get( TAU_SOS_MAP_COMMUNICATOR(comm), ndims );
...
MPI_Comm * comm_graph;
{
  ...
  TAU_TRACK_COMM(TAU_SOS_MAP_COMMUNICATOR(comm_old));
  returnVal = PMPI_Graph_create( TAU_SOS_MAP_COMMUNICATOR(comm_old), nnodes, index, edges, reorder, comm_graph );
...
int * edges;
{
  ...
  TAU_TRACK_COMM(TAU_SOS_MAP_COMMUNICATOR(comm));
  returnVal = PMPI_Graph_get( TAU_SOS_MAP_COMMUNICATOR(comm), maxindex, maxedges, index, edges );
...
int * newrank;
{
  ...
  returnVal = PMPI_Graph_map( TAU_SOS_MAP_COMMUNICATOR(comm_old), nnodes, index, edges, newrank );
...
int * neighbors;
{
  ...
  TAU_TRACK_COMM(TAU_SOS_MAP_COMMUNICATOR(comm));
  returnVal = PMPI_Graph_neighbors( TAU_SOS_MAP_COMMUNICATOR(comm), rank, maxneighbors, neighbors );
...
int * nneighbors;
{
  ...
  TAU_TRACK_COMM(TAU_SOS_MAP_COMMUNICATOR(comm));
  returnVal = PMPI_Graph_neighbors_count( TAU_SOS_MAP_COMMUNICATOR(comm), rank, nneighbors );
...
int * nedges;
{
  ...
  TAU_TRACK_COMM(TAU_SOS_MAP_COMMUNICATOR(comm));
  returnVal = PMPI_Graphdims_get( TAU_SOS_MAP_COMMUNICATOR(comm), nnodes, nedges );
...
int * top_type;
{
  ...
  TAU_TRACK_COMM(TAU_SOS_MAP_COMMUNICATOR(comm));
  returnVal = PMPI_Topo_test( TAU_SOS_MAP_COMMUNICATOR(comm), top_type );
...
int TauGetCpuSite(int *node, int *core, int *rank) {
  ...
  PMPI_Comm_size(TAU_SOS_MAP_COMMUNICATOR(MPI_COMM_WORLD), &nprocs);
  ...
  for (n=0; n<nprocs; n++) {
    PMPI_Bcast(&(host_names[n]),MPI_MAX_PROCESSOR_NAME, MPI_CHAR, n, TAU_SOS_MAP_COMMUNICATOR(MPI_COMM_WORLD)); 
  }
  ...
  PMPI_Comm_split(TAU_SOS_MAP_COMMUNICATOR(MPI_COMM_WORLD), color, *rank, &internode);
  PMPI_Comm_rank(internode,core);
  
  PMPI_Comm_split(TAU_SOS_MAP_COMMUNICATOR(MPI_COMM_WORLD), *core, *rank, &intranode);
...
char * Tau_printRanks(void *comm_ptr) {
  ...
  PMPI_Comm_size(TAU_SOS_MAP_COMMUNICATOR(comm), &size);
  limit = (size < TAU_MAX_MPI_RANKS) ? size : TAU_MAX_MPI_RANKS;
  for ( i = 0; i < limit; i++) {
    worldrank = TauTranslateRankToWorld(TAU_SOS_MAP_COMMUNICATOR(comm), i);
    if (i == 0) { 
